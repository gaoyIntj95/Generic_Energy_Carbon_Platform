...
function InventoryReferencePageV2({ project, activities, confirmed, onActivitiesChange, onConfirm }: { project: Project; activities: ActivityDataRow[]; confirmed: boolean; onActivitiesChange: (rows: ActivityDataRow[]) => void; onConfirm: (rows: ActivityDataRow[]) => void }) {
  ...
  return <div className={inventoryStyles.page}>
    ...
    <div className={inventoryStyles.workspace}>
      <aside className={inventoryStyles.side}>
        <div className={inventoryStyles.sideTitle}>生命周期</div>
        {groups.map((stage) => <div className={inventoryStyles.group} key={stage.title}>
          <div className={inventoryStyles.groupTitle}>{stage.title}</div>
          {stage.keys.map((key) => {
            const active = group === key;
            return <button key={key} className={`${inventoryStyles.nav} ${active ? inventoryStyles.navActive : ''}`} style={{ width: '100%', border: 0, background: active ? '#eaf7f2' : 'transparent', font: 'inherit' }} onClick={() => { setGroup(key); resetQuery(); }}><span>{activityGroups.find((item) => item.key === key)?.label}</span><span className={inventoryStyles.badge}>{activities.filter((row) => row.category === groupCategory[key]).length}</span></button>;
          })}
        </div>)}
      </aside>
...
function ReferenceActivityDrawer({ type, group, row, mode = 'add', onClose, onSubmit }: { type: ActivityType; group?: ActivityGroupKey; row?: ActivityDataRow; mode?: 'add' | 'data'; onClose: () => void; onSubmit: (row: ActivityDataRow) => void }) {
  ...
  if (type === '工艺排放') return <div className={styles.overlay} onMouseDown={onClose}>
    <form className={styles.modal} style={{ width: 'min(760px, 92vw)', maxHeight: 'calc(100vh - 48px)' }} onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}>
      <header><div><div className={inventoryStyles.drawerTitle}>{mode === 'data' ? '补充生产过程排放' : '新增生产过程排放'}</div><small>每个排放活动对应固定的排放量获取方式，用户仅填写当前活动所需数据。</small></div><button className={inventoryStyles.close} type="button" onClick={onClose}>×</button></header>
  ...
  return <div className={styles.overlay} onMouseDown={onClose}>
    <form className={styles.modal} style={{ width: 'min(760px, 92vw)', maxHeight: 'calc(100vh - 48px)' }} onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}>
      <header><div><div className={inventoryStyles.drawerTitle}>{mode === 'data' ? '补充活动数据' : `新增${current.label}`}</div><small>系统根据当前生命周期阶段自动归类</small></div><button className={inventoryStyles.close} type="button" onClick={onClose}>×</button></header>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '0 22px', maxHeight: 'calc(100vh - 190px)', overflowY: 'auto' }}>
        <div style={{ gridColumn: '1 / -1' }} className={inventoryStyles.section}>活动数据</div>
        ...
        <div className={inventoryStyles.form}><label className={inventoryStyles.label}>{nonProcessType === '运输' ? '运输重量' : '用量'} {requiredMark}</label><div className={inventoryStyles.grid2}><input className={inventoryStyles.field} required type="number" min="0" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="请输入数值" /><select className={inventoryStyles.select} value={unit} onChange={(event) => setUnit(event.target.value)}>{(nonProcessType === '运输' ? ['t', 'kg'] : unitOptions).map((option) => <option key={option}>{option}</option>)}</select></div>{unit === '自定义' && <input className={inventoryStyles.field} required value={customUnit} onChange={(event) => setCustomUnit(event.target.value)} placeholder="请输入自定义单位，例如：箱、批" style={{ marginTop: 10 }} />}</div>
        ...
        {nonProcessType === '运输' && <div className={inventoryStyles.form} style={{ gridColumn: '1 / -1' }}><label className={inventoryStyles.label}>运输参数 {requiredMark}</label><div className={inventoryStyles.gridTransport}><select className={inventoryStyles.select} value={transportMode} onChange={(event) => { setTransportMode(event.target.value); setFactor(''); setFactorValue(''); }}>{transportModes.map((option) => <option key={option}>{option}</option>)}</select><input className={inventoryStyles.field} required type="number" min="0" step="0.01" value={transportDistance} onChange={(event) => setTransportDistance(event.target.value)} placeholder="运输距离（km）" /></div><div className={inventoryStyles.calc}>运输周转量：<b>{amount && transportDistance ? `${transportTurnover.toFixed(4)} t·km` : '待根据运输重量和距离计算'}</b><span className={inventoryStyles.minor}>按运输重量折算为吨后 × 运输距离（km）自动计算，并作为排放因子匹配与核算依据。</span></div></div>}
...
function ProjectModal({ project, onClose, onSubmit }: { project?: Project; onClose: () => void; onSubmit: (project: Pick<Project, 'name' | 'spec' | 'category' | 'unit' | 'boundary' | 'period' | 'processFile'>) => void }) {
  ...
  const [category, setCategory] = useState(project?.category ?? '机械设备');
  ...
  return <div className={styles.overlay} onMouseDown={onClose}><form className={styles.modal} onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); if (startDate > endDate) return; onSubmit({ name, spec, category, unit, boundary: boundary.split('（')[0], period, processFile }); }}><header><b>{project ? '编辑产品碳足迹项目' : '新建产品碳足迹项目'}</b><button type="button" onClick={onClose}>×</button></header><div>
    <h3>产品信息</h3><div className={styles.formGrid}><label>产品名称<input required value={name} onChange={(e) => setName(e.target.value)} placeholder="请输入产品名称" /></label><label>规格型号<input required value={spec} onChange={(e) => setSpec(e.target.value)} placeholder="请输入产品规格型号" /></label><label>产品类别<select value={category} onChange={(e) => setCategory(e.target.value)}><option>机械设备</option><option>金属制品</option><option>电子电器</option></select></label><label>功能单位<input value={unit} onChange={(e) => setUnit(e.target.value)} /></label></div>
