...
export function Modal({ title, description, children, onClose, onSubmit, footer, width = 680, submitText = '保存', cancelText = '取消', dataEntryMode, variant = 'default' }: {
  title: string; description?: ReactNode; children: ReactNode; onClose: () => void; onSubmit?: () => void; footer?: ReactNode; width?: number; submitText?: string; cancelText?: string; dataEntryMode?: string; variant?: 'default' | 'delete';
}) {
  ...
  return <div className={s.overlay} onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <form className={`${s.modal} ${variant === 'delete' ? s.deleteModal : ''}`} role="dialog" aria-modal="true" data-entry-mode={dataEntryMode} style={{ width }} onSubmit={submit}>
