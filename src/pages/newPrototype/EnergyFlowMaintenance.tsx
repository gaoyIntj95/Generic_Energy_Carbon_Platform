...
import {
  ...
  type V11ConversionOutput, type V11ExternalSupplyRecord,
...
function AnnualFlowMaintenance() {
  ...
  const [expandedConversionId, setExpandedConversionId] = useState('');
  ...
  const editingConversion = conversions.find((row) => row.conversionOutputId === editingConversionId);
  ...
  return <div className={s.page}>
    <p className={s.hint} role="note"><strong>{year} 年度独立维护</strong> · 列表展示年度汇总，查看展开只读月度明细，编辑通过弹窗维护，修改不影响其他年度。</p>
    ...
    {editor && <FlowRecordDialog key={`${editor.kind}:${editor.kind === 'external' ? editor.item?.externalSupplyId ?? 'new' : editor.unitId}:${editingMonth}:${version}`} editor={editor} year={year} index={editingMonth - 1} onClose={() => setEditor(null)} onSaved={() => { setEditor(null); refresh('记录已保存'); }} />}
    {deleting && <FlowModal variant="delete" title={deleting.kind === 'conversion' ? '删除产出与损失数据' : '删除外供数据'} width={680} onClose={() => setDeleting(null)} footer={<><Button onClick={() => setDeleting(null)}>取消</Button><Button primary type="submit">确认删除</Button></>} onSubmit={() => { if (deleting.kind === 'conversion') deleteFlowConversionData(deleting.id, deleting.month); else deleteFlowExternalData(deleting.id, deleting.month); setDeleting(null); refresh('所选期间数据已删除'); }}><div className={s.deleteNotice}><p>确认删除“<strong>{deleting.name}</strong>”{deletionPeriod}的{deleting.kind === 'conversion' ? '产出与损失数据' : '外供数据'}？{deleting.month ? '其他月份保留。' : '将清除该年度全部月份。'}</p></div></FlowModal>}
    {confirmation}
    <Toast message={message} />
  </div>;
}

function ConversionDetails({ row, initialMonth, onClose, onSaved, onSource }: { row: V11ConversionOutput; initialMonth: number; onClose: () => void; onSaved: (text: string) => void; onSource: (month: number) => void }) {
  ...
  const annualInput = inputAmount(row, null) ?? '';
  const annualOutput = outputs.some((value) => value !== '') ? sum(outputs.map((value) => Number(value || 0))) : outputChanged ? '' : outputAmount(row, null) ?? '';
  const annualLoss = losses.some((value) => value !== '') ? sum(losses.map((value) => Number(value || 0))) : lossChanged ? '' : lossAmount(row, null) ?? '';
...
function FlowRecordDialog({ editor, year, index, onClose, onSaved }: { editor: Editor; year: number; index: number; onClose: () => void; onSaved: () => void }) {
  ...
  const annualAmount = amounts.some((value) => value !== '') ? sum(amounts.map((value) => Number(value || 0))) : quantityChanged ? '' : e?.amount ?? '';
  ...
  const save = () => {
    if (kind === 'external') {
      ...
      const scene = conversionScenarioFor({ inputEnergyTypeId: source?.energyTypeId ?? recoveryType, outputEnergyTypeId: outputType }, types);
  ...
  return <FlowModal title={`${kind === 'external' ? e ? '编辑外供数据' : '登记外供' : '补数据'}（${year}年度）`} description="年度独立维护，保存仅影响当前年度数据。" width={760} onClose={onClose} onSubmit={save} submitText="保存"><div className={s.form}>
    ...
    {kind === 'external' ? <><div className={s.formGrid}><Field label="供能来源" required><select aria-label="供能来源" disabled={Boolean(e) && !sourceChanged && quantityChanged} value={externalSource} onChange={(event) => { const next = event.target.value; setExternalSource(next); setAmounts(initialAmounts.map((value, i) => next === (e?.conversionOutputId ?? e?.inputEnergyRecordId) || i !== sourceMonth ? value : '')); }}><option value="">请选择来源</option><optgroup label="用能单元转换与回收产出">{conversions.map((r) => <option key={r.conversionOutputId} value={r.conversionOutputId}>{v11ScopeName(r.conversionEnergyUnitId, year)} · {r.outputEnergyName}</option>)}</optgroup><optgroup label="外购能源直接外供">{records.filter((r) => v11RecordScopeType(r) === 'enterprise' && r.energyRole === '能源消费').map((r) => <option key={r.energyRecordId} value={r.energyRecordId}>{types.find((t) => t.energyTypeId === r.energyTypeId)?.energyTypeName} · {r.year}年企业能源消费</option>)}</optgroup></select></Field><Field label="外供能源"><input readOnly value={externalType ? `${externalType.energyTypeName} · ${externalConversion?.outputUnit ?? externalType.measurementUnit}` : '选择来源后带出'} /></Field><Field label="接收方" required><input aria-label="接收方" required value={receiver} onChange={(event) => setReceiver(event.target.value)} /></Field></div>{Boolean(e) && !sourceChanged && quantityChanged && <p className={s.hint}>保存数量修改后可调整供能来源。</p>}{sourceChanged && <><Field label="调整来源月份"><select aria-label="调整来源月份" value={sourceMonth} onChange={(event) => { const month = Number(event.target.value); setSourceMonth(month); setAmounts(initialAmounts.map((value, i) => i === month ? '' : value)); }}>{Array.from({ length: 12 }, (_, i) => <option key={i} value={i}>{i + 1}月</option>)}</select></Field><p className={s.hint}>仅将所选月份调整至新来源并重新填写数量，其余月份保留原来源。</p></>}
      ...
      </div><div className={s.monthInputGrid}>{amounts.map((value, i) => <Field key={i} label={`${i + 1}月产出（${types.find((type) => type.energyTypeId === outputType)?.measurementUnit ?? '—'}）`}><input aria-label={`${i + 1}月产出量`} type="number" min="0" step="any" value={value} onChange={(event) => setAmounts((current) => current.map((item, j) => i === j ? event.target.value : item))} /></Field>)}</div><div className={s.annualSummary}><Field label={`年度产出汇总${types.find((type) => type.energyTypeId === outputType)?.measurementUnit ? `（${types.find((type) => type.energyTypeId === outputType)?.measurementUnit}）` : ''}`}><input aria-label="年度产出汇总" readOnly value={annualAmount} /></Field></div></>}
...
import { conversionScenarioFor } from '../../modules/data-management/energyConversionRelations';
